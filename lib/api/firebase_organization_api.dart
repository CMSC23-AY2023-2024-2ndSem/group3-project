...
class FirebaseOrganizationAPI {
  ...
  //   try {
  //     await db.collection("organization").add(todo);
  ...
  Stream<QuerySnapshot> getOrganizationByName(String? name) {
    return db.collection("organizations").where("name", isEqualTo: name).snapshots(); 
  }

  // Future<String> deleteTodo(String id) async {
  //   try {
  //     await db.collection("organization").doc(id).delete();
